<template>
  <section class="page">
    <AdminPanel title="作品管理" subtitle="查看并处理作品状态">
      ...
      <div class="list-scroll">
        ...
        <div v-if="!loading && artworks.length === 0" class="empty-state">还没有作品</div>
      </div>
    ...
    <AdminModal :visible="detailVisible" @close="closeDetail">
      <div class="detail-shell">
        <div class="detail-image-wrap">
          ...
          />
          <div v-else class="detail-empty">{{ detailLoading ? '详情加载中...' : '暂无预览' }}</div>
        </div>
        ...
        <section class="detail-side">
          ...
          <template v-else-if="detailData">
            <div class="detail-head">
              <div>
                <p class="detail-kicker">作品ID：{{ detailData.id }}</p>
...
.info-card {
  @include panel-surface($radius: 16px, $border: #e3e8df, $background: #f8faf7, $shadow: none);
  padding: 12px 14px;
}
...
.comment-item {
  @include panel-surface($radius: 16px, $border: #e2e7df, $background: #fbfcfa, $shadow: none);
  padding: 12px 14px;
}
