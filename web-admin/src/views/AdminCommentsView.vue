<template>
  <section class="page">
    <AdminPanel title="评论管理" subtitle="基于已公开接口聚合作品评论并执行删除">
      <AdminSearchBar v-model="keyword" placeholder="搜索评论内容 / 用户名 / 作品标题">
        <button type="button" class="ghost-btn" @click="reloadList">刷新列表</button>
      </AdminSearchBar>
      ...
      <div class="meta-row">
        <span>已加载 {{ comments.length }} 条评论</span>
      ...
      <div class="list-scroll">
        ...
        >
          <div class="row-main">
            <div class="row-top">
              <strong class="row-title">评论 #{{ item.id }}</strong>
              <span class="status-tag">{{ item.artworkVisibilityLabel }}</span>
            </div>
            <p class="row-content">{{ item.content }}</p>
            <p class="row-subline">
              {{ item.username }} · 作品《{{ item.artworkTitle }}》 · 作品 #{{ item.artworkId }}
            </p>
          </div>
          <div class="row-stats">
        ...
        <div v-if="!loading && filteredComments.length === 0" class="empty-state">暂无评论数据</div>
      </div>
      ...
      <div class="detail-shell">
        ...
        <section class="detail-side">
          <div v-if="detailComment">
            <p class="detail-kicker">评论 #{{ detailComment.id }}</p>
            ...
            <div class="reason-box">
              <label class="field-label" for="delete-reason">删除备注</label>
              <textarea
                id="delete-reason"
                v-model.trim="deleteReason"
                class="reason-input"
                maxlength="200"
                placeholder="可选填写删除原因"
              ></textarea>
            </div>
